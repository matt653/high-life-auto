...
const VehicleEditor: React.FC<VehicleEditorProps> = ({ vehicle, onSave, onClose }) => {
  const [formData, setFormData] = useState<Vehicle>({ ...vehicle });
  const [isAnalyzing, setIsAnalyzing] = useState(false);
  const [activeTab, setActiveTab] = useState<'details' | 'ai'>('ai');
                  ...
                  {formData.aiGrade ? (
                    <div className="space-y-6">
                         ...
                         <div className="space-y-3">
                            {Object.values(formData.aiGrade.categories).map((cat: any, idx) => (

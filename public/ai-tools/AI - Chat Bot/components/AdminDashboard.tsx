...
import { Lead, InventoryItem, KnowledgeItem, Message } from '../types';
...
const getHeatAttributes = (status: string) => {
...
interface StageModalProps {
  ...
  lead: Lead;
  onClose: () => void;
  onConfirm: (data: Partial<Lead>) => void;
}

const StageRequirementModal: React.FC<StageModalProps> = ({ stageId, lead, onClose, onConfirm }) => {
...
const ManualLeadForm: React.FC<ManualLeadFormProps> = ({ userId, onClose, demoMode, onAdd }) => {
...
// --- EDIT LEAD FORM ---
const EditLeadForm: React.FC<{ userId: string, lead: Lead, onClose: () => void, demoMode: boolean, onUpdate: (id: string, updates: Partial<Lead>) => void }> = ({ userId, lead, onClose, demoMode, onUpdate }) => {
...
const SalesPipeline: React.FC<PipelineProps> = ({ leads, userId, onSelectLead, demoMode, onAddLead, onUpdateLead }) => {
  const [showManualAdd, setShowManualAdd] = useState(false);
  const [editingLead, setEditingLead] = useState<Lead | null>(null);
  const [stageModal, setStageModal] = useState<{ lead: Lead, stageId: string } | null>(null);

  const handleStageClick = (lead: Lead, stageId: string) => {
    // Open modal to confirm or add data
    setStageModal({ lead, stageId });
  };

  const handleStageConfirm = async (updates: Partial<Lead>) => {
    if (!stageModal) return;
    const { lead, stageId } = stageModal;
    
    const finalUpdates = {
        ...updates,
        status: stageId as any,
        updatedAt: demoMode ? { seconds: Date.now() / 1000 } : serverTimestamp()
    };
                             ...
                             <div className="flex items-center gap-1">
                                {PIPELINE_STAGES.slice(0, 7).map((stage, idx) => {
                                  ...
                                  return (
                                    <button
                                      ...
                                      className={`h-8 w-8 rounded-md border flex items-center justify-center transition-all shadow-sm ${
                                        isFilled 
                                ...
                                <button
                                  ...
                                  className={`h-8 w-8 rounded-full flex items-center justify-center transition-all shadow-sm ${
                                    isSold ? 'bg-green-500 text-white ring-2 ring-green-200' : 'bg-white border border-gray-200 text-gray-300 hover:text-green-500 hover:border-green-500'
                                  }`}
                                  title="Mark Sold"
                                >
                                ...
                                <button
                                  ...
                                  className={`h-8 w-8 rounded-full flex items-center justify-center transition-all shadow-sm ${
                                    isLost ? 'bg-gray-800 text-white ring-2 ring-gray-200' : 'bg-white border border-gray-200 text-gray-300 hover:text-red-500 hover:border-red-500'
                                  }`}
                                  title="Mark Lost"
                                >
                                  <Ghost size={14} />
...
const TranscriptViewer: React.FC<{ lead: Lead, onClose: () => void }> = ({ lead, onClose }) => {
...
const ChatLogs: React.FC<{ leads: Lead[], onSelectLead: (l: Lead) => void }> = ({ leads, onSelectLead }) => (
...
const KnowledgeManager: React.FC<{ knowledge: KnowledgeItem[], demoMode: boolean }> = ({ knowledge, demoMode }) => {
      ...
      <div className="space-y-4">
        {knowledge.map(rule => (
          <div key={rule.id} className="bg-white p-6 rounded-[2rem] border shadow-sm flex items-start gap-4 border-l-8 border-l-orange-500">
            ...
            <button 
              onClick={() => {
                if (demoMode) alert("Deletion simulated.");
                else deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'knowledge', rule.id));
              }} 
...
export const AdminDashboard: React.FC<AdminProps> = ({ user, leads, inventory, knowledge, integrations, onSync, isSyncing, demoMode, onAddLead, onUpdateLead }) => {
  const [activeTab, setActiveTab] = useState<'pipeline' | 'chats' | 'knowledge'>('pipeline');
  const [selectedLead, setSelectedLead] = useState<Lead | null>(null);

  const TabButton = ({ id, icon: Icon, label }: any) => (
    <button 
      onClick={() => { setActiveTab(id); setSelectedLead(null); }}
    ...
    <div className="flex w-full h-full overflow-hidden bg-slate-50">
      ...
      <aside className="w-64 bg-gray-950 text-white p-4 flex flex-col shrink-0 border-r border-white/5">
        <div className="mb-8 px-4 pt-4">
           ...
           <div className="text-[9px] text-gray-500 font-bold uppercase tracking-[0.2em] mt-1 pl-8">Admin Console</div>
           {demoMode && <div className="mt-2 text-[8px] bg-red-900 text-red-200 px-2 py-1 rounded text-center">DEMO MODE ACTIVE</div>}
        </div>
        
        <TabButton id="pipeline" icon={CheckCircle2} label="The Pipeline" />
        <TabButton id="chats" icon={MessageCircle} label="Chats" />
        <TabButton id="knowledge" icon={ShieldAlert} label="Sales Rules" />

        <div className="mt-auto pt-6 border-t border-white/10 space-y-3">
          <button onClick={onSync} disabled={isSyncing} className="w-full flex items-center justify-center gap-2 bg-white/10 hover:bg-white/20 text-white py-3 rounded-xl text-[10px] font-black uppercase transition-all">
      ...
      <section className="flex-1 overflow-hidden relative">
        <div className="absolute inset-0 overflow-y-auto p-8">
           {selectedLead && activeTab === 'pipeline' ? (
             <div className="max-w-4xl mx-auto animate-fade-in-up">
               <button onClick={() => setSelectedLead(null)} className="flex items-center text-gray-400 font-black uppercase text-[10px] tracking-widest mb-6 hover:text-gray-900">
                 <ChevronRight className="rotate-180 mr-1" size={14} /> Back to Pipeline
               </button>
               {/* Simplified Lead View for Pipeline context */}
               <div className="bg-white p-8 rounded-[3rem] shadow-xl border">
                 <h1 className="text-3xl font-black uppercase italic mb-2">{selectedLead.name}</h1>
                 <p className="text-gray-500 font-medium mb-8">Lead ID: {selectedLead.id}</p>
                 <div className="bg-slate-50 p-6 rounded-2xl">
                    <h3 className="font-black uppercase text-xs text-gray-400 tracking-widest mb-4">Last Activity</h3>
                    <p className="italic text-gray-700 font-medium">"{selectedLead.lastMessage}"</p>
                    <div className="mt-6">
                      <h4 className="font-black uppercase text-xs text-gray-400 tracking-widest mb-2">Move Phase</h4>
                      <select 
                        disabled
                        value={selectedLead.status}
                        className="w-full bg-gray-100 p-3 rounded-xl border-r-[16px] border-transparent outline-none font-bold opacity-50"
                      >
                         {PIPELINE_STAGES.map(s => <option key={s.id} value={s.id}>{s.title}</option>)}
                      </select>
                      <p className="text-[10px] text-gray-400 mt-2 italic">Status is managed in the table view.</p>
                    </div>
                 </div>
               </div>
             </div>
             ...
             <>
               {activeTab === 'pipeline' && <SalesPipeline leads={leads} userId={user.uid} onSelectLead={setSelectedLead} demoMode={demoMode} onAddLead={onAddLead} onUpdateLead={onUpdateLead} />}
               {activeTab === 'chats' && <ChatLogs leads={leads} onSelectLead={setSelectedLead} />}
               {activeTab === 'knowledge' && <KnowledgeManager knowledge={knowledge} demoMode={demoMode} />}
             </>
           )}
           
           {/* GLOBAL MODAL FOR CHAT TRANSCRIPTS (If selected via Chats Tab) */}
